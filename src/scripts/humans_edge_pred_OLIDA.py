...
edge_to_predict = ('GENE', 'OLIDA', 'GENE')

# 1st for_loop
for target_of_script in ['OLIDA']: # 'proven_SL' 
    # 2nd for_loop
    for train_type in ['unbalanced']: # on edge_pred, doing a 'balanced' train set is useless
        # 3rd for_loop
        for features in ['basic', 'strict', 'large']:

            base_model_param = f"edge_pred_genes_19330_p2p_OLIDA_train_{train_type}_features_{features}_STRING_{string_threshold}_{ana_edges}_ana_{activation}_abcm_{node_mode}_2HetConv_{heteroconv_aggr_1}_{heteroconv_aggr_2}_SAGE_{sage_aggr}_sagenorm_{sage_norm}"
            ...
            gene_strict_tensor, target_tensor, _ = get_strict_target_tensors(target_to_use = target_of_script, features_to_use = features, remove_sex_chrom = False)

            def create_abcmg_data(list_of_edges: list, use_edge_attr: bool = False, feature_mode: str = 'degree', merge_choice: bool = True, my_target: str = 'essential', features_type: str = 'basic'):
            ...
            def plot_df_edges(df_edges: pd.DataFrame, val_set: bool = False):
                # In this setting, the val set sometimes contains only positive (or only negative) class
            ...
            for i in [16,32,64]:
                ...
                print("XXX" * 40)
                print("This is Humans: link prediction setting on OLIDA pairs.")

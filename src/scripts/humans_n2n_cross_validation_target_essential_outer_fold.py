...
## this script will perform the proper outter cross_validation by retraining models with best hyperparameters on inner folds
...
result_path = current_path + f'humans/n2n/outer_cross_val/'
model_path = current_path + f'humans/n2n/outer_cross_val/'
...
def add_strict_nodes(data: HeteroData, node_target: str = 'essential', features_type: str = 'basic') -> HeteroData:
    gene_strict_tensor, target_tensor, _ = get_strict_target_tensors(target_to_use = node_target, features_to_use = features_type)
...
def add_strict_nodes(data: HeteroData, target_to_use, features_type) -> HeteroData:
    gene_strict_tensor, target_tensor, _ = get_strict_target_tensors(target_to_use = target_to_use, remove_sex_chrom = False, features_to_use = features_type)
    data['GENE'].x = gene_strict_tensor.to(torch.float32)
    data['GENE'].y = target_tensor.type(torch.float32)
    # configure RandomNodesSplit to have balanced dataset during training 
    # attention à num_val
    data = T.RandomNodeSplit(split = 'random', num_train_per_class = 1500, num_val = 6389, num_test = 10000, key = 'y')(data)
    #data = T.RandomNodeSplit(split = 'train_rest', num_val = 0.1, num_test = 0.2, key = 'y')(data)
    for type_of_node in nodes_types:
        if type_of_node != 'GENE':
            data[type_of_node].num_nodes = len(nodes2id[type_of_node])
    return data
...
def create_masks_cross_val(data: HeteroData, split_seed: int = 1, cv_fold_to_test: int = 1) -> HeteroData:
    ...
    if cv_fold_to_test == 'leave_out_test':
        print("I will be training on the 10-folds and testing on the ledt-out test set.")
        train_folds = ['cv_1', 'cv_2', 'cv_3', 'cv_4', 'cv_5', 'cv_6', 'cv_7', 'cv_8', 'cv_9', 'cv_10']
        test_fold = 'leave_out_test'
    else:
        cv_fold_to_test = f"cv_seed_{split_seed}"
        train_folds = ['cv_1', 'cv_2', 'cv_3', 'cv_4', 'cv_5', 'cv_6', 'cv_7', 'cv_8', 'cv_9', 'cv_10']
...
data = HeteroData()
data = add_strict_nodes(data, target_to_use = target_of_script, features_type = features)
...
gene_strict_tensor, target_tensor, _ = get_strict_target_tensors(target_to_use = target_of_script, remove_sex_chrom = False, features_to_use = features)
...
nodes = nodes[['GENE_id', 'mapped_id', 'y']]
...
cross_val_dict = {
    1: 'relu_32', 2: 'relu_32', 3: 'relu_16', 4: 'relu_64', 5: 'relu_16', 6: 'relu_16',
    7: 'relu_16', 8: 'relu_32', 9: 'relu_32', 10: 'relu_16', 11: 'relu_16', 12: 'relu_32', 
    13: 'relu_32', 14: 'relu_32', 15: 'relu_16', 16: 'relu_16', 17: 'relu_16', 18: 'relu_16', 
    19: 'relu_64', 20: 'relu_16', 21: 'relu_16', 22: 'relu_32', 23: 'relu_64', 24: 'relu_32', 
    25: 'relu_16', 26: 'relu_16', 27: 'relu_32', 28: 'relu_32', 29: 'relu_16', 30: 'relu_32'
    }

for split_seed, params in cross_val_dict.items():
    print("xxxx"*40)
    print(f"For the seed: {split_seed}")
    activation = params.split('_')[0]
    dim = int(params.split('_')[1])

    model_param = f"humans_19330_n2n_essential_features_{features}_STRING_700_aggr1_{heteroconv_aggr_1}_aggr2_{heteroconv_aggr_2}_cn_2_SAGE_sageaggr_{sage_aggr}_sagenorm_{sage_norm}_batch_{size_of_batch}_trainneighbors_{neighbors}_cross_val_split_seed_{split_seed}_act_{activation}_dim_{dim}_{dim}"
    ...
    # TODO
    # the create_masks_cross_val function has been updated
    j = 'leave_out_test'
    data = create_masks_cross_val(data = data, split_seed = split_seed, cv_fold_to_test = j)

    pred_df.loc[:, ['train']] = data['GENE'].train_mask.numpy().astype(float)
    pred_df.loc[:, ['test']] = data['GENE'].test_mask.numpy().astype(float) 
    pred_df.loc[pred_df[pred_df['train']==1].index, ['mask']] = 'train'
    pred_df.loc[pred_df[pred_df['test']==1].index, ['mask']] = 'test'
    ...
    model_name = model_path + model_param + f'_epoch_final_{best_epoch}.pyg'
    ...
    fig_name = result_path + model_param + f'_epoch_final_{best_epoch}.png'
    ...
    file_name = result_path + model_param + f"_explanation.txt"
    f = open(file_name, "w")
    f.write(f"This nodes_to_nodes model is trained on Humans, target = 'essential' from Bartha et al., outter cross_validation:")
    f. write(f"The features used come from this dataframe: 'data/processed/GRL4DD_feat_chrom_20230414.tsv'\n")
    f.write(f"The features used are: {features}")
    f.write(f"For the models: {model_param}, for a maximum of {epochs_to_train}, the edges that were used are:\n")
    for edge in data.edge_index_dict.keys():
        f.write(f"{edge} \n")

...
result_path = current_path + f'/results/humans/n2n_pLI/'
model_path = current_path + f'/models/humans/n2n_pLI/'
        ...
        for size_of_batch in [128]:
            ...
            for activation in ['tanh', 'relu']: # 
                ...
                if conv_type == 'SAGE':
                    heads = 'not_concerned'
                    if conv_number == 1:
                        base_model_param = f"humans_19330_n2n_pLI_cn_{conv_number}_aggr1_{heteroconv_aggr_1}_conv_SAGE_sageaggr_{sage_aggr}_sagenorm_{sage_norm}_act_{activation}"
                    elif conv_number == 2:
                            base_model_param = f"humans_19330_n2n_pLI_feat_{features}_cn_{conv_number}_aggr1_{heteroconv_aggr_1}_aggr2_{heteroconv_aggr_2}_conv_SAGE_sageaggr_{sage_aggr}_sagenorm_{sage_norm}_act_{activation}"
                    elif conv_number == 3:
                        heteroconv_aggr_3 = hyperparameters['heteroconv_aggr_3']
                        base_model_param = f"humans_19330_n2n_pLI_cn_{conv_number}_aggr1_{heteroconv_aggr_1}_aggr2_{heteroconv_aggr_2}_aggr3_{heteroconv_aggr_3}_conv_SAGE_sageaggr_{sage_aggr}_sagenorm_{sage_norm}_act_{activation}"
                elif conv_type == 'GATv2':
                    heads = hyperparameters['Heads']
                    sage_norm = 'not_concerned'
                    sage_aggr = 'not_concerned'
                    if conv_number == 1:
                        base_model_param = f"humans_19330_n2n_pLI_cn_{conv_number}_aggr1_{heteroconv_aggr_1}_conv_GATv2_heads_{heads}_act_{activation}"
                    elif conv_number == 2:
                        heteroconv_aggr_2 = hyperparameters['heteroconv_aggr_2']
                        base_model_param = f"humans_19330_n2n_pLI_cn_{conv_number}_aggr1_{heteroconv_aggr_1}_aggr2_{heteroconv_aggr_2}_conv_GATv2_heads_{heads}_act_{activation}"
                    elif conv_number == 3:
                        heteroconv_aggr_3 = hyperparameters['heteroconv_aggr_3']
                        base_model_param = f"humans_19330_n2n_pLI_cn_{conv_number}_aggr1_{heteroconv_aggr_1}_aggr2_{heteroconv_aggr_2}_aggr3_{heteroconv_aggr_3}_conv_GATv2_heads_{heads}_act_{activation}"
                ...
                feat = pd.read_csv(data_processed_path + 'human_genes_features_20230414.tsv', sep = '\t', header = 0)
                feat = pd.merge(nodes, feat, left_on = 'GENE_id', right_on = 'ENSG_ID', how = 'left')

                nodes = feat[['GENE_id', 'mapped_id', 'pLI']]

                nodes.loc[nodes[nodes['pLI'] >= 0.9].index, ['y']] = 1
                nodes.loc[nodes[nodes['pLI'] < 0.9].index, 'y'] = 0
                nodes.loc[nodes[nodes['pLI'].isna()].index, 'y'] = 0
                nodes.loc[nodes[nodes['pLI'] == -1].index, 'y'] = 0
                nodes = nodes.rename(columns={'y':'target'})
                nodes.loc[:,['type']] = 'haplosensitive'
                nodes.loc[nodes[nodes['target'] == 0].index, 'type'] = 'haplosufficient'
                ...
                for i in [16, 32, 64]: 
                    ...
                    for j in range(1,31): 
                        ...
                        print("==="*40)
                        print(f"Now using: seed_{j}")

                        df_fold_name = 'humans_19330_n2n_pLI_splits_seeds.tsv'
                        cv_folds = pd.read_csv(data_processed_path + df_fold_name, sep = '\t', header = 0)
                        col_to_use = f"seed_{j}"
                        folds = cv_folds[[col_to_use]]
                        folds = folds.rename(columns = {col_to_use: f'fold_{j}'})
                        col_to_use = f'fold_{j}'

                        fd = fd[basic_columns]
                        fd = pd.concat([fd, folds], axis=1)
                        # get the indexes
                        train_idx = folds[folds[col_to_use] == 'train'].index
                        test_idx = folds[folds[col_to_use] == 'test'].index
                        # initialize masks
                        ...
                        print("XXX" * 40)
                        print("This is Humans: nodes_to_nodes setting, target = haplosensitive vs haplosufficient.")

                        if conv_number == 1:
                            print(f"Now hidden_channels_1 = {hc_1} and batch = {size_of_batch}.")
                            outter_model_param = base_model_param + f"_{hc_1}_batch_{size_of_batch}_neighbors_{neighbors}_global_all_mask_{dict_j[j]}"
                            hc_2 = 'not_concerned'
                            heteroconv_aggr_2 = 'not_concerned'
                        elif conv_number == 2:
                            print(f"Now hidden_channels_1 = {hc_1} and hidden_channels_2 = {hc_2} and batch = {size_of_batch}.")
                            outter_model_param = base_model_param + f"_{hc_1}_{hc_2}_batch_{size_of_batch}_neighbors_{neighbors}_global_all_mask_{dict_j[j]}"
                        elif conv_number == 3:
                            print(f"Now hidden_channels_1 = {hc_1} and hidden_channels_2 = {hc_2} and hidden_channels_3 = {hc_3} and batch = {size_of_batch}.")
                            outter_model_param = base_model_param + f"_{hc_1}_{hc_2}_{hc_3}_batch_{size_of_batch}_neighbors_{neighbors}_global_all_mask_{dict_j[j]}"
                        else:
                        ...
                        print(fd)
                        pred_df = fd[['GENE_id', 'mapped_id', 'pLI', 'target', 'type', f'fold_{j}']]
                        # pred_df = fd.copy()
                        full_AUROC_df = pd.DataFrame(
                        {'epoch': int(), 'AUROC_train': float(), 'train_loss': float(), 'AUROC_val': float(), 'val_loss': float(),
                        'AUROC_test': float(), 'test_loss': float() #, 'mask': int(), 'best_auroc_test_epoch': int() , 'params': str(),
                        }, index = [])

                        # TODO
                        if 'val_mask' in data['GENE']:
                        ...
                        fig, df_tmp, fusion_tmp = plot_model_df_v4(my_model, global_loader = global_loader, zero_out_batch = zero_out_batch_features, 
                                                        val_set = False, test_set = True, setting = 'nodes_to_nodes')
                    ...
                    f = open(file_name, "w")
                    f.write(f"This nodes_to_nodes model is trained on Humans:")
                    ...
                    f.write("We didn't use the FUSIL classification features.")

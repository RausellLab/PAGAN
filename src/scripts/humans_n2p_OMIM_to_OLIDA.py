...
hyperparameters = {
    ...
    # 'size_of_batch': 128,
    'neighbors': 30,
    'disjoint_loader' : True, # p2p = no need to disjoint_loader, the features for pairs will be 0 
...
remove_doublons = hyperparameters['remove_doublons']
...
result_path = current_path + '/results/humans/n2p_OMIM_OLIDA/'
model_path = current_path + '/models/humans/n2p_OMIM_OLIDA/'

# the activation for_loop is below
# 1st for_loop
for conv_type in ['SAGE']: #
    # 2nd for_loop
    for features in ['basic', 'strict', 'large']: #
        
        prefix_model_param = f"humans_19330_n2p_OMIM_OLIDA_feat_{features}_cn_{conv_number}_aggr1_{heteroconv_aggr_1}_aggr2_{heteroconv_aggr_2}_conv_SAGE_sageaggr_{sage_aggr}_sagenorm_{sage_norm}"
        ...
        def create_masks(data: HeteroData, df: pd.DataFrame, use_val_set: bool = False) -> HeteroData:
            # get the indexes
            if use_val_set:
                val_idx = df[df['mask'] == 'val'].index 
            train_idx = df[df['mask'] == 'train'].index 
            test_idx = df[df['mask'] == 'test'].index 
            # initialize masks
        ...
        data = HeteroData()
        data = add_strict_nodes(data, node_target = target_of_script, features_type = features)
        ...
        gene_strict_tensor, target_tensor, _ = get_strict_target_tensors(target_to_use = target_of_script, features_to_use=features)

        
        def create_abcmg_data(list_of_edges: list, use_edge_attr: bool = False, feature_mode: str = 'degree', merge_choice: bool = True, my_target: str = 'essential', features_type = 'basic'): 
            ...
            gene_strict_tensor, target_tensor, _ = get_strict_target_tensors(target_to_use = my_target, features_to_use = features_type, remove_sex_chrom = False)
            bool_attr = use_edge_attr
            data = HeteroData()
            data = add_strict_nodes(data, node_target = my_target, features_type = features_type)
        ...
        # harmonize this function with plot_model_df from the other scripts
        # specific behavior of val_set in the p2p setting, final val_set = concat of val_set and test_set 

        def plot_model_df_v4(model_to_test, global_loader, zero_out_batch: bool = True, val_set: bool = True, test_set: bool = True, setting: str = 'nodes_to_nodes'):
        ...
        if ana_edges in ['all', 'common']:
            ...
            list_of_edges, data = create_abcmg_data(['expressed_in', 'ana_ana',
                                                ...
                                                'gene_to_MF', 'molecular_function'
                                                    ], feature_mode = node_mode, use_edge_attr = edge_features, merge_choice = False, my_target = target_of_script, features_type = features) # 
        else: 
        ...
        list_of_edges_2, data_2 = create_abcmg_data(['PPI', 'paralog' #, 'CoExp', 'Neighbour', 
                                                    ], use_edge_attr = edge_features, merge_choice = True, my_target = target_of_script, features_type = features) # 
        ...
        pairs_df_name = 'OLIDA_dataset.tsv'
        print(pairs_df_name)
        fd = pd.read_csv(data_processed_path + pairs_df_name, header=0, sep='\t', dtype={6:str, 7:str})
        if remove_doublons == True:
            print(f'The initihal shape of the df is: {fd.shape}')
            fd_initial_shape = len(fd)
            fd = fd.drop_duplicates(subset='GENE_id', keep='first', ignore_index=True)
            fd.loc[:,'mapped_id'] = pd.RangeIndex(len(fd))
            fd_new_shape = len(fd)
            print(f"I deleted {fd_initial_shape-fd_new_shape} rows because they were doublons.")
        else:
            print("I am keeping all the duplicated pairs.")
            fd.loc[:,'mapped_id'] = pd.RangeIndex(len(fd))
        fd['target'] = fd['target'].astype(int)

        # rewrite the target and the mask
        fd = fd.drop('target', axis = 1)
        fd = fd.merge(nodes[['mapped_id', 'OMIM_target', 'target']], how='left', left_on='mapped_id', right_on='mapped_id')
        # change masks
        fd.loc[fd[fd['GENE_id'].str.contains('_')].index, 'mask'] = 'test'
        fd.loc[fd[(fd['set']=='individual_gene')&(fd['OMIM_target'].isin([0,1]))].index, 'mask'] = 'train'
        # change target and OMIM target
        fd.loc[fd[fd['type']=='OLIDA'].index, 'target'] = 1
        fd.loc[fd[fd['type']=='OLIDA'].index, 'OMIM_target'] = -1
        fd.loc[fd[fd['type'].isin(['FREX', 'set_neg'])].index, 'target'] = 0
        fd.loc[fd[fd['type'].isin(['FREX', 'set_neg'])].index, 'OMIM_target'] = -1
        # change type
        fd.loc[fd[(fd['set']=='individual_gene') & (fd['OMIM_target'] == 1)].index, 'type'] = 'mmdg'
        fd.loc[fd[(fd['set']=='individual_gene') & (fd['OMIM_target'] == 0)].index, 'type'] = 'no_OMIM_disease'
        fd.loc[fd[(fd['set']=='individual_gene') & (fd['OMIM_target'] == -1)].index, 'type'] = 'OMIM_unknown'
        # change dtype for target and OMIM target
        fd['OMIM_target'] = fd['OMIM_target'].astype(int)
        fd['target'] = fd['target'].astype(int)
        # get basic columns
        ...
        # use nodesid['GENE'] for heterogeneous edge_types, no forbidden values there
        ...
        # data['GENE'].n_id = torch.arange(data['GENE'].num_nodes)
        ...
        # create mask
        data = create_masks(data = data, df = fd, use_val_set = False)

        ## outter_loop, outter-loop
        ## 3rd for_loop
        for activation in ['tanh', 'relu']: # 

            base_model_param = prefix_model_param + f"_{activation}_abcm_{node_mode}_2HetConv_{heteroconv_aggr_1}_{heteroconv_aggr_2}_SAGE_{sage_aggr}_sagenorm_{sage_norm}"
            ...
            ## indent begins here 
                ...
                conv_number = hyperparameters['conv_number']    # 2
                sage_aggr = hyperparameters['sage_aggr'] # 'sum'
                sage_norm = hyperparameters['sage_norm']
                ...
                print("XXX" * 40)
                print("This is Humans: nodes_to_pairs setting, target = OMIM for individual genes and OLIDA for pairs.")
                ...
                if setting == 'nodes_to_nodes':
                    pred_df = fd[['GENE_id', 'mapped_id', 'OMIM_target', 'target']]
                else: 
                ...
                my_model, best_epoch, AUROC_df = do_complete_training_loader(my_model, optimizer_to_use = my_optimizer, criterion_to_stop = 'train_loss', 
                                                                            val_set = False, test_set = True, remove_self_features = zero_out_batch_features,
                ...
                fig, df_tmp, fusion_tmp = plot_model_df_v4(my_model, global_loader = global_loader, zero_out_batch = zero_out_batch_features, 
                                                        val_set = False, test_set = True, setting = 'pairs_to_pairs')
            ...
            _, _, dfeat = get_strict_target_tensors(target_to_use = target_of_script, remove_sex_chrom = False, features_to_use=features)
            file_name = result_path + outter_model_param + f"_explanation.txt"
            f = open(file_name, "w")
            f.write(f"This nodes_to_pairs model is trained on Humans, on individual genes that are OMIM MMDG and on OLIDA pairs")
            ...
            if features == 'basic':
                f.write("I didn't use the FUSIL category.")

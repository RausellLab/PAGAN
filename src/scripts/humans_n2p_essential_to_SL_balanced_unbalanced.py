...
hyperparameters = {
    ...
    # 'size_of_batch': 128,
    'neighbors': 30,
    'disjoint_loader' : True, #False
    'zero_out_batch_features': True,
    'epochs_to_train' : 500, #50, 100
    ...
    'shuffle_train': True,
    'ana_edges': 'common', # 'all', 'common'
    'string_threshold': 700, # confidence = {'low': 0.15, 'medium': 0.4, 'high': 0.7, 'highest': 0.9} from STRING doc
    'global_loader_neighbors': 'all',
...
result_path = current_path + '/results/humans/p2p/'
model_path = current_path + '/models/humans/p2p/'

# 1st for_loop
for target_of_script in ['essential_A375', 'essential_A549', 'essential_K562']: #
    if target_of_script == 'essential_K562':
        SL_type = 'K562_SL'
        SNL_types = ['unmatched', 'matched']
    elif target_of_script == 'essential_A375':
        SL_type = 'A375_SL'
        SNL_types = ['unmatched', 'matched'] 
    elif target_of_script == 'essential_A549':
        SL_type = 'A549_SL'
        SNL_types = ['unmatched', 'matched']
    else:
        raise Exception(f"Please select either 'essential_K562' or 'essential_A375' or 'essential_A549' for the target to use in the n2p approach.")
    # 2nd for_loop
    for activation in ['relu', 'tanh']:
        SNL_type = 'mixed'
        # 3rd for_loop
        for train_set in ['balanced', 'unbalanced']: # other script for ['all_ind']
            ## indent to here <----------
            base_model_param = f"genes_19330_n2p_{SL_type}_{SNL_type}_SNL_train_{train_set}_STRING_{string_threshold}_{ana_edges}_ana_{activation}_abcm_{node_mode}_2HetConv_{heteroconv_aggr_1}_{heteroconv_aggr_2}_SAGE_{sage_aggr}_sagenorm_{sage_norm}"
            ...
            def plot_model_df(model_to_test, global_loader, zero_out_batch: bool = True, val_set: bool = True, test_set: bool = True, setting: str = 'nodes_to_pairs'):
            ...
            ## open fd file
            ## TODO fusion the matched and unmatched df into one 
            df_pairs_matched_name = f'20250717_SL_matched_SNL_{target_of_script}_-1_disjoint_sets_splits_05_08.tsv'
            df_pairs_unmatched_name = f'20250717_SL_unmatched_SNL_{target_of_script}_-1_disjoint_sets_splits_05_08.tsv'
            fd_1 = pd.read_csv(data_processed_path + df_pairs_matched_name, header=0, sep='\t', dtype={6:str, 7:str})
            fd_2 = pd.read_csv(data_processed_path + df_pairs_unmatched_name, header=0, sep='\t', dtype={6:str, 7:str})

            nodes = fd_1[~fd_1['GENE_id'].str.contains('_')]
            pairs = fd_1[fd_1['GENE_id'].str.contains('_')]
            pairs.loc[pairs[pairs['type'] == 'SNL'].index, ['SNL_type']] = 'matched'
            snl_2 = fd_2[fd_2['type']=='SNL']
            snl_2.loc[snl_2[snl_2['type'] == 'SNL'].index, ['SNL_type']] = 'unmatched'

            all_pairs = pd.concat([pairs, snl_2], axis=0)
            all_pairs = all_pairs.sort_values(by = 'GENE_id', ignore_index=True)
            fd = pd.concat([nodes, all_pairs], axis = 0)
            fd = fd.reset_index(drop=True)
            fd['mapped_id'] = pd.RangeIndex(len(fd))
            print(f'The shape of the pairs_df is: {fd.shape}')

            essential_genes = fd[(fd['in_pairs_05']=='non_sampled')&(fd['type']=='essential')]['GENE_id'].tolist()
            pairs_genes = list(set(fd[fd['type'].isin(['SL', 'SNL'])]['left_side'].unique().tolist()+fd[fd['type'].isin(['SL', 'SNL'])]['right_side'].unique().tolist()))
            pairs_genes.sort()
            print(f"There are {len(pairs_genes)} genes used in the pairs.")
            non_essential_genes = fd[fd['type'] == 'non_essential']['GENE_id'].unique().tolist()
            non_essential_genes.sort()
            unused_non_essential_genes = list(set(non_essential_genes) - set(pairs_genes))
            unused_non_essential_genes.sort()
            print(f"There are: {len(essential_genes)} essential_genes and: {len(unused_non_essential_genes)} non_essential_genes that are not present in any pairs and can be used for training.")

            if train_set == 'balanced':
                print(f'I will now randomly sampled {len(essential_genes)} non_essential genes from the {len(unused_non_essential_genes)} available.')
                random.seed(42)
                sampled_names = random.sample(unused_non_essential_genes, k = len(essential_genes))
                sampled_names.sort()
                fd.loc[:, ['mask']] = 'individual_genes'
                fd.loc[fd[fd['GENE_id'].isin(essential_genes)].index, ['mask']] = 'train'
                fd.loc[fd[fd['GENE_id'].isin(sampled_names)].index, ['mask']] = 'train'
                fd.loc[fd[fd['GENE_id'].str.contains('_')].index, ['mask']] = 'test'
            else: # train_set == 'unbalanced'
                print(f'I will now use all the individual genes that are not present in any pair as a training set for this n2p approach.')
                fd.loc[:, ['mask']] = 'individual_genes'
                fd.loc[fd[fd['GENE_id'].isin(essential_genes)].index, ['mask']] = 'train'
                fd.loc[fd[fd['GENE_id'].isin(unused_non_essential_genes)].index, ['mask']] = 'train'
                fd.loc[fd[fd['GENE_id'].str.contains('_')].index, ['mask']] = 'test'
            ...
            new_mask = torch.zeros(size = (fd.shape[0] - data['GENE'].train_mask.shape[0],))
            new_global_mask = torch.ones(size = (fd.shape[0],))
            new_mask = new_mask > 1
            new_global_mask = new_global_mask > 0
            data['GENE'].x = torch.cat(tensors = (data['GENE'].x, new_features), axis = 0)
            data['GENE'].y = new_target

            print("I am training on all individual genes.")
            fd.loc[:,['mask']] = 'train'
            fd.loc[fd[fd['GENE_id'].str.contains('_')].index, 'mask'] = 'test'
            ...
            ## indent begins here 
                ...
                hl_2 = hyperparameters['hidden_linear_2']       # 16
                ...
                print("XXX" * 40)
                print("This is Humans: nodess_to_pairs setting, target = essential vs non_essential for individual genes and SL vs SNL for pairs.")
                ...
                'mask': str(), 'epoch': str(), 'dim': int()}, index = [])
                ...
                'AUROC_test': float(), 'test_loss': float(), 'mask': int(), 'epoch': int(), 'best_auroc_test_epoch': int() #, 'params': str(),
                }, index = [])

                col_names_of_pred_df = []
                best_col_names_of_pred_df = []

                def create_masks_essential_pairs(data: HeteroData, use_val_set:bool = True) -> HeteroData:
                    # get the indexes
                ...
                data = create_masks_essential_pairs(data, use_val_set = False)
                ...
                # my_model = HeteroConv_1SAGE(hidden_channels_1 = hc_1, out_channels = 1, sage_aggr = sage_aggr, sage_norm = sage_norm, sage_project = False, hetero_aggr_1 = heteroconv_aggr_1)  
                if activation == 'tanh':
                ...
                fig, df_tmp, fusion_tmp = plot_model_df(my_model, global_loader = global_loader, zero_out_batch = zero_out_batch_features, 
                                                        val_set = False, test_set = True, setting = 'nodes_to_pairs')
            ...
            ## explanation 
            ## indent ends here for the inner gor_loop
            ...
            f = open(file_name, "w")
            f.write(f"This nodes_to_pairs model is trained on Humans, with {target_of_script} for individual genes.")
            f. write(f"The features used come from this dataframe: 'data/processed/GRL4DD_feat_chrom_20230414.tsv'\n")
            f.write(f"The features used are:")
            f.write("I didn't use the FUSIL features, nor the CoExp or Neighbour edges.")

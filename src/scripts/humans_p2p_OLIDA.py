...
# 1st for_loop
for train_type in ['unbalanced', 'balanced']: # 'balanced' gives poor performances on OLIDA
    # 2nd for_loop
    for features in ['basic', 'strict', 'large']: #
        if remove_doublons:
            prefix_model_param = f"genes_19330_p2p_OLIDA_train_{train_type}_features_{features}_STRING_{string_threshold}_{ana_edges}_ana"
        elif remove_doublons == False:
            prefix_model_param = f"genes_19330_p2p_OLIDA_keep_doublons_train_{train_type}_features_{features}_STRING_{string_threshold}_{ana_edges}_ana"
        ...
        if train_type == 'unbalanced':
            pass
        elif train_type == 'balanced':
            positive_train_set = fd[(fd['target']==1)&(fd['mask']=='train')].shape[0]
            print(f'I will use as many negative pairs as there are positive pairs in the training set, which is: {positive_train_set}.')
            index_to_sample = list(fd[(fd['target']==0)&(fd['mask']=='train')].index)
            fd.loc[fd[(fd['target']==0)&(fd['mask']=='train')].index, 'mask'] = 'val'
            random.seed(42)
            new_negative_train_set = random.sample(index_to_sample, k=positive_train_set)
            new_negative_train_set.sort()
            fd.loc[new_negative_train_set, 'mask'] = 'train'
        ...
        for activation in ['tanh', 'relu']: # 
            ...
            ## indent begins here 
                ...
                print("XXX" * 40)
                print("This is Humans: pairs_to_pairs setting, target = OLIDA for pairs.")
            ...
            f = open(file_name, "w")
            f.write(f"This pairs_to_pairs model is trained on Humans, on OLIDA pairs")
            ...
            if features == 'basic':
                f.write("I didn't use the FUSIL.")

...
result_path = current_path + f'/results/yeasts/edge_pred/'
model_path = current_path + f'/models/yeasts/edge_pred/'
    ...
    for DMF_t in [0.5, 0.4, 0.3]: # 
        ...
        for splits in ['50_50', '80_20']:
            ...
            for activation in ['tanh', 'relu']:
                ...
                #     base_model_param = f"yeasts_edge_pred_{edge_pred_setting}_{sl_genes}_SL_genes_DMF_{DMF_t}_SNL_split_{splits}_STRING_{string_threshold}_ana_{activation}_bcm_{node_mode}_2HetConv_{heteroconv_aggr_1}_{heteroconv_aggr_2}_SAGE_{sage_aggr}_sagenorm_{sage_norm}"
                # elif doublons_pairs == 'remove_all':
                base_model_param = f"yeasts_edge_pred_no_doublons_{edge_pred_setting}_{sl_genes}_SL_genes_DMF_{DMF_t}_SNL_split_{splits}_STRING_{string_threshold}_ana_{activation}_bcm_{node_mode}_2HetConv_{heteroconv_aggr_1}_{heteroconv_aggr_2}_SAGE_{sage_aggr}_sagenorm_{sage_norm}"
                ...
                class HeteroConv_2SAGE_act(torch.nn.Module):
                    def __init__(self, hidden_channels_1, hidden_channels_2, sage_aggr = 'sum', sage_norm = True, sage_project = False, hetero_aggr_1 = 'sum', hetero_aggr_2 = 'sum', activation = 'relu'): # num_layers, hidden_linear_2
                        super().__init__()
                ...
                ## indent begins here 
                    ...
                    print("XXX" * 40)
                    print("This is Yeasts: link prediction setting on synthetic_lethality_edges.")
                    ...
                    model, best_epoch, AUROC_df = do_complete_edge_training(model, optimizer_to_use = optimizer, criterion_to_stop = 'train_loss', 
                                                                                        val_set = False, test_set = False, edge_type = edge_to_predict,
                ...
                f.write(f"This edge_prediction model is trained on Yeasts, on {edge_to_predict} edges.")

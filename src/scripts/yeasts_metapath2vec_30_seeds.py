...
result_path = current_path + f'/results/yeasts/baseline/'
model_path = current_path + f'/models/yeasts/baseline/'
...
yeast_features_to_use = [
    'dn_ds', 
    # 'chemical_compound_accumulation','chronological_lifespan', 'competitive_fitness',
...
model_path = current_path + '/models/yeasts/baseline/'
# wl = walk_length, cs = context_size, wpn = walk_per_node, nns = num_nnegative_sample
model_name = 'yeasts_baseline_metapath2vec_n2n_wl_100_cs_10_wpn_20_nns_5' + f'_split_seed_{split_seed}_epoch_{epoch}.pyg'
...
metapath_tot = pd.DataFrame(data = {'AUROC_test': AUROC_test_metapath, 'AUPR_test': AUPR_test_metapath, 'model': 'Metapath2vec', 'random_seed':split_seed}, index=[split_seed]) 
...
for split_seed in range(2,31):
    ...
    AUROC_test_metapath = roc_auc_score(test_targets, test_preds[:,1])
    AUPR_test_metapath = average_precision_score(test_targets, test_preds[:,1])
    metapath_tmp = pd.DataFrame(data = {'AUROC_test': AUROC_test_metapath, 'AUPR_test': AUPR_test_metapath, 'model': 'Metapath2vec', 'random_seed':split_seed}, index=[split_seed]) 
    metapath_tot = pd.concat([metapath_tot, metapath_tmp], axis=0)
    print("xxx")

results_path = '/results/yeasts/baseline/'
df_name = 'yeasts_baseline_metapath2vec_30_seeds.tsv'

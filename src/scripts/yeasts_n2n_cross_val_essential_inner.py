...
for split_seed in range(1,31):
    ...
    result_path = current_path + f'/results/yeasts/n2n/seed_{split_seed}/'
    model_path = current_path + f'/models/yeasts/n2n/seed_{split_seed}/'

    # 2nd for_loop
    for conv_type in ['SAGE']:   # ['SAGE', 'GATv2']
        # 3rd for_loop
        for conv_number in [2]: # [1,2,3]
            # 4th for_loop
            for size_of_batch in [128]: # [16, 32, 64, 128]
                # 5th for_loop
                for activation in ['relu', 'tanh']: # 
                    ...
                    if conv_type == 'SAGE':
                        heads = 'not_concerned'
                        if conv_number == 1:
                            heteroconv_aggr_3 = hyperparameters['heteroconv_aggr_1']
                            base_model_param = f"yeasts_n2n_essential_split_{split_seed}_STRING_{string_threshold}_cn_{conv_number}_aggr1_{heteroconv_aggr_1}_conv_SAGE_sageaggr_{sage_aggr}_sagenorm_{sage_norm}_act_{activation}"
                        elif conv_number == 2:
                            heteroconv_aggr_3 = hyperparameters['heteroconv_aggr_2']
                            base_model_param = f"yeasts_n2n_essential_split_{split_seed}_STRING_{string_threshold}_cn_{conv_number}_aggr1_{heteroconv_aggr_1}_aggr2_{heteroconv_aggr_2}_conv_SAGE_sageaggr_{sage_aggr}_sagenorm_{sage_norm}_act_{activation}"
                        elif conv_number == 3:
                            heteroconv_aggr_3 = hyperparameters['heteroconv_aggr_3']
                            base_model_param = f"yeasts_n2n_essential_split_{split_seed}_STRING_{string_threshold}_cn_{conv_number}_aggr1_{heteroconv_aggr_1}_aggr2_{heteroconv_aggr_2}_aggr3_{heteroconv_aggr_3}_conv_SAGE_sageaggr_{sage_aggr}_sagenorm_{sage_norm}_act_{activation}"
                    elif conv_type == 'GATv2':
                        sage_norm = 'not_concerned'
                        sage_aggr = 'not_concerned'
                        if conv_number == 1:
                            heteroconv_aggr_3 = hyperparameters['heteroconv_aggr_1']
                            base_model_param = f"yeasts_n2n_essential_split_{split_seed}_STRING_{string_threshold}_cn_{conv_number}_aggr1_{heteroconv_aggr_1}_conv_GATv2_heads_{heads}_act_{activation}"
                        elif conv_number == 2:
                            heteroconv_aggr_3 = hyperparameters['heteroconv_aggr_2']
                            base_model_param = f"yeasts_n2n_essential_split_{split_seed}_STRING_{string_threshold}_cn_{conv_number}_aggr1_{heteroconv_aggr_1}_aggr2_{heteroconv_aggr_2}_conv_GATv2_heads_{heads}_act_{activation}"
                        elif conv_number == 3:
                            heteroconv_aggr_3 = hyperparameters['heteroconv_aggr_3']
                            base_model_param = f"yeasts_n2n_essential_split_{split_seed}_STRING_{string_threshold}_cn_{conv_number}_aggr1_{heteroconv_aggr_1}_aggr2_{heteroconv_aggr_2}_aggr3_{heteroconv_aggr_3}_conv_GATv2_heads_{heads}_act_{activation}"
                    ...
                    for i in [16,32,64]: 
                        ...
                        print("XXX" * 40)
                        print(f"This is Yeasts: {setting} setting, target = essential vs non_essential.")
                        
                        if conv_number == 1:
                            print(f"Now hidden_channels_1 = {hc_1} and batch = {size_of_batch}.")
                            outter_model_param = base_model_param + f"_{hc_1}_batch_{size_of_batch}_neighbors_{neighbors}"
                            hc_2 = 'not_concerned'
                            heteroconv_aggr_2 = 'not_concerned'
                        elif conv_number == 2:
                            print(f"Now hidden_channels_1 = {hc_1} and hidden_channels_2 = {hc_2} and batch = {size_of_batch}.")
                            outter_model_param = base_model_param + f"_{hc_1}_{hc_2}_batch_{size_of_batch}_neighbors_{neighbors}"
                        elif conv_number == 3:
                            print(f"Now hidden_channels_1 = {hc_1} and hidden_channels_2 = {hc_2} and hidden_channels_3 = {hc_3} and batch = {size_of_batch}.")
                            outter_model_param = base_model_param + f"_{hc_1}_{hc_2}_{hc_3}_batch_{size_of_batch}_neighbors_{neighbors}"
                        else:
                            raise Exception("Please select a conv_number in [1,2,3].")
                        
                        if global_loader_neighbors == 'all':
                            outter_model_param = outter_model_param + f"_global_all"
                        else:
                            outter_model_param = outter_model_param + f"_global_sample"
                        ...
                        for j in range(1,11): 
                            ...
                            # I do not want to coalesce = keep the edges_doublons for pairs
                            for edge in data.edge_index_dict.keys():
                                data[edge].edge_index = data[edge].edge_index.contiguous()
                                data[edge].edge_index = U.sort_edge_index(data[edge].edge_index)
                        ...
                        f = open(file_name, "w")
                        f.write(f"This nodes_to_nodes model is trained on Yeasts: the gene target is essential vs non-essential.")

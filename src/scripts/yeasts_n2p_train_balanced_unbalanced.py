...
result_path = current_path + f'/results/yeasts/n2p/'
model_path = current_path + f'/models/yeasts/n2p/'

# 1st for_loop
for sl_genes in [4261, 3841]: #
    # 2nd for_loop
    for activation in ['tanh', 'relu']:
        DMF_t = 0.5 # only use the big datasdet for the n2p appproach, to begin with 
        # 3rd for_loop
        for train_set in ['unbalanced', 'balanced']: 
            # if doublons_pairs == 'keep_first':
            #     base_model_param = f"yeasts_n2p_{sl_genes}_SL_genes_DMF_{DMF_t}_no_interaction_train_{train_set}_workers_{num_workers}_STRING_{string_threshold}_ana_{activation}_bcm_{node_mode}_2HetConv_{heteroconv_aggr_1}_{heteroconv_aggr_2}_SAGE_{sage_aggr}_sagenorm_{sage_norm}"
            # elif doublons_pairs == 'remove_all':
            base_model_param = f"yeasts_n2p_no_doublons_{sl_genes}_SL_genes_DMF_{DMF_t}_no_interaction_train_{train_set}_workers_{num_workers}_STRING_{string_threshold}_ana_{activation}_bcm_{node_mode}_2HetConv_{heteroconv_aggr_1}_{heteroconv_aggr_2}_SAGE_{sage_aggr}_sagenorm_{sage_norm}"
            ...
            ## individual nodes target

            essential_genes = fd[(fd['in_pairs_05']=='not_in_pairs')&(fd['type']=='essential')]['GENE_id'].tolist()
            unused_non_essential_genes = fd[(fd['in_pairs_05']=='not_in_pairs')&(fd['type']=='non_essential')]['GENE_id'].tolist()
            ...
            if sl_genes == 3841: #in [3841,3853]:
                if train_set == 'balanced':
                    ...
                    fd.loc[fd[fd['GENE_id'].str.contains('_')].index, ['mask']] = 'test'
            elif sl_genes == 4261: # in [4262,4261]:
                if train_set == 'balanced':
                    print(f'I will now randomly sampled {len(unused_non_essential_genes)} essential genes from the {len(essential_genes)} available.')
                    random.seed(42)
                    sampled_names = random.sample(essential_genes, k = len(unused_non_essential_genes))
                    sampled_names.sort()
                    fd.loc[:, ['mask']] = 'individual_genes'
                    fd.loc[fd[fd['GENE_id'].isin(unused_non_essential_genes)].index, ['mask']] = 'train'
                    fd.loc[fd[fd['GENE_id'].isin(sampled_names)].index, ['mask']] = 'train'
            ...
            fd_end[['GENE_id', 'mapped_id', 'left_gene_to_CC_dst_nodes', 'right_gene_to_CC_dst_nodes']]

            # fd_end[['GENE_id', 'mapped_id', 'mapped_id_left', 'mapped_id_right', 'left_gene_to_ANATOMY_dst_nodes', 'right_gene_to_ANATOMY_dst_nodes']]
            ...
            ## indent begins here 
                ...
                print("XXX" * 40)
                print("This is Yeasts: nodes_to_pairs setting, target = essential vs non-essential for individual genes and SL vs SNL for pairs.")
                ...
                my_model, best_epoch, AUROC_df = do_complete_training_loader(my_model, optimizer_to_use = my_optimizer, criterion_to_stop = 'train_loss', 
                                                                            val_set = False, 
                                                                            test_set = False, # False, True
                                                                            remove_self_features = zero_out_batch_features,
                ...
                fig_name = path + outter_model_param + f'_epoch_final_{best_epoch}.png'
            ...
            f = open(file_name, "w")
            f.write(f"This nodes_to_pairs model is trained on Yeasts, with DMF score < {DMF_t} for SL pairs and random sampling for SNL pairs:")
